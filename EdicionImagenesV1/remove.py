...
class ProcessingSettings:
    ...
    output_dir: str = 'processed_products'  # Directorio de salida
...
class FastBackgroundRemover:
    ...
    def __init__(self, backend='u2netp', quality_mode='balanced'):
        ...
        - u2netp: Más rápido que u2net, buena calidad (recomendado)
        ...
        - 'balanced': Alpha matting optimizado (recomendado)
...
class ObjectAnalyzer:
    ...
    def get_object_bounds_fast(image: Image.Image) -> Optional[Tuple[int, int, int, int]]:
        ...
        if not bbox:
            # Fallback a método numpy si falla
...
class Model2Processor:
    ...
    def process_single_image(self, image: Image.Image) -> Optional[Image.Image]:
        ...
        try:
            ...
            if orientation == ObjectOrientation.VERTICAL:
                return self._apply_vertical_rule(image_no_bg, bbox)
            elif orientation == ObjectOrientation.HORIZONTAL:
                return self._apply_horizontal_rule(image_no_bg, bbox)
            elif orientation == ObjectOrientation.SQUARE:
                return self._apply_square_rule(image_no_bg, bbox)
            else:
                return self._apply_no_object_rule(image_no_bg)
    ...
    def _apply_vertical_rule(self, image: Image.Image, bbox: Tuple[int, int, int, int]) -> Image.Image:
        """Regla 1: Objeto vertical - márgenes arriba/abajo"""
        x1, y1, x2, y2 = bbox
        obj_width = x2 - x1
        obj_height = y2 - y1
        scale_with_margins = min(
            self.settings.target_width / obj_width,
            (self.settings.target_height - 2 * self.settings.margin_pixels) / obj_height
        )
    ...
    def _apply_horizontal_rule(self, image: Image.Image, bbox: Tuple[int, int, int, int]) -> Image.Image:
        """Regla 2: Objeto horizontal - márgenes izquierda/derecha"""
        x1, y1, x2, y2 = bbox
        obj_width = x2 - x1
        obj_height = y2 - y1
        scale_with_margins = min(
            (self.settings.target_width - 2 * self.settings.margin_pixels) / obj_width,
    ...
    def _apply_square_rule(self, image: Image.Image, bbox: Tuple[int, int, int, int]) -> Image.Image:
        """Objeto cuadrado - márgenes uniformes"""
        x1, y1, x2, y2 = bbox
        obj_width = x2 - x1
        obj_height = y2 - y1
        obj_size = max(obj_width, obj_height)
        scale = min(
            (self.settings.target_width - 2 * self.settings.margin_pixels) / obj_size,
            (self.settings.target_height - 2 * self.settings.margin_pixels) / obj_size
        )
    ...
    def _apply_no_object_rule(self, image: Image.Image) -> Image.Image:
        """Regla 3: Sin objeto detectado - centrar imagen completa"""
...
if __name__ == "__main__":
    ...
    print("⚖️ MODO BALANCEADO (mejor relación calidad/velocidad)")
